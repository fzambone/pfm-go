...
- **Always run integration tests via `make test-integration`, never `go test -tags integration` directly.** The local container runtime is OrbStack (Docker-compatible). Integration tests must pass locally via `make test-integration` **before** pushing — do not rely on CI to catch integration failures.
